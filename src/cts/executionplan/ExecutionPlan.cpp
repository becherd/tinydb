...
void ExecutionPlan::generateExecutionPlan() {
	...
	for (unsigned int i = 1; i < query.size(); i++) {
		...
		cross.push_back(
				unique_ptr<CrossProduct>(
						new CrossProduct(move(cross.at(i - 1)),
								move(query.at(i)))));
	}

	//do Join
	vector<unique_ptr<Operator>> join;
	join.push_back(move(cross.at(cross.size() - 1)));
	for (unsigned int i = 0; i < r.joinConditions.size(); i++) {
		const Register* joinAttribute1 = getRegister(
				r.joinConditions.at(i).first);
		const Register* joinAttribute2 = getRegister(
				r.joinConditions.at(i).second);

		join.push_back(
				unique_ptr<Selection>(
						new Selection(move(join.at(i)), joinAttribute1,
								joinAttribute2)));
	}
	...
	unique_ptr<Projection> projection = unique_ptr<Projection>(
			new Projection(move(join.at(join.size() - 1)), projectionRegister));
